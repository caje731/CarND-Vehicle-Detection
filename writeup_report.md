...
I used the [Moving Mean algorithm](https://en.wikipedia.org/wiki/Moving_average) to recalculate the heat map in every frame. I use 90% of the last frame heat map and use 10% of the heatmap of the new frame. This way the heatmap between the frames is smoothened. I also used both upper and lower threshold for the heat map. The same Moving Average algorithm is used to recompute the already detected car's average centroid, width and height in every frame. One difficulty I faced was with fine tuning the parameters for extracting the features - It took more time to manually try different combinations of them. Similarly identifying the right window size and the number of windows was time consuming.
